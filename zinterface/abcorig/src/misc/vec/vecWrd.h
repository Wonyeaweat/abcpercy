...
/*
 * Provide a lightweight local implementation of Abc_RandomW when the
 * project does not supply it. This avoids an "not declared" error
 * while keeping behavior reasonable for tests/builds.
 *
 * This implementation is deterministic and respects the "seed" parameter.
 * - If seed != 0, the internal PRNG is (re)seeded with the provided value.
 * - If seed == 0, the PRNG continues from its previous state.
