...
////////////////////////////////////////////////////////////////////////
// Co-root collection helpers
////////////////////////////////////////////////////////////////////////
// collector for co-root annotation strings
static Vec_Ptr_t* s_pCoRootGroups = NULL;


static void Io_CoRootInit()
{
    if (s_pCoRootGroups)
        Vec_PtrFree(s_pCoRootGroups);
    s_pCoRootGroups = Vec_PtrAlloc(1000);
}

static void Io_CoRootAdd(Abc_Obj_t* pNode)
{
    int n = pNode->vCoRoots.nSize;
    if (n < 2)
        return;
    ...
    int pos = 0;
    if (n == 2)
        pos += snprintf(buf + pos, sizeof(buf) - pos, "#dual");
    else
        pos += snprintf(buf + pos, sizeof(buf) - pos, "#Triple");
    // append each co-root's output name if available, otherwise the id
    for (int i = 0; i < n && pos < (int)sizeof(buf) - 1; i++)
    {
        int id = pNode->vCoRoots.pArray[i];
        Abc_Obj_t* pNode2 = Abc_NtkObj(Abc_ObjNtk(pNode), id);
        const char* pOutName = NULL;
        if (pNode2)
        {
            Abc_Obj_t* pOut = Abc_ObjFanout0(pNode2);
            if (pOut)
                pOutName = Abc_ObjName(pOut);
        }
        if (pOutName)
            pos += snprintf(buf + pos, sizeof(buf) - pos, " %s", pOutName);
        else
            pos += snprintf(buf + pos, sizeof(buf) - pos, " %d", id);
    }
    // store the string (make a copy)
    if (s_pCoRootGroups)
    {
        /* deduplicate: skip if an identical annotation already recorded */
        int i, n = Vec_PtrSize(s_pCoRootGroups);
        for (i = 0; i < n; i++)
        {
            char* s = (char*)Vec_PtrEntry(s_pCoRootGroups, i);
            if (s && strcmp(s, buf) == 0)
                return; /* already present */
        }
        Vec_PtrPush(s_pCoRootGroups, strdup(buf));
    }
}

static void Io_CoRootWriteAndFree(FILE* pFile)
{
    if (s_pCoRootGroups == NULL)
        return;
    // write a separation line before annotations
    if (Vec_PtrSize(s_pCoRootGroups) > 0)
    {
        fprintf(pFile, "\n");
        for (int i = 0; i < Vec_PtrSize(s_pCoRootGroups); i++)
        {
            char* s = (char*)Vec_PtrEntry(s_pCoRootGroups, i);
            if (s)
                fprintf(pFile, "%s\n", s);
...
// helpers to collect co-root groups and print them at the end of the BLIF
static void Io_CoRootInit();
static void Io_CoRootAdd(Abc_Obj_t* pNode);
static void Io_CoRootWriteAndFree(FILE* pFile);
...
void Io_WriteBlif(Abc_Ntk_t* pNtk, char* FileName, int fWriteLatches, int fBb2Wb, int fSeq)
{
    ...
    // init co-root collection (we will append annotations at the end of the file)
    Io_CoRootInit();
...
//    assert( Abc_NtkWhiteboxNum(pNtk) == 0 );
    ...
    // write collected co-root annotations at the end of the file
    Io_CoRootWriteAndFree(pFile);
    fclose(pFile);
}
...
void Io_NtkWriteNodeFanins(FILE* pFile, Abc_Obj_t* pNode)
{
    ...
    fprintf(pFile, " %s", pName);
    /* collect co-root groups for later annotation at end of file
     * for size==2 and size==3 we do not emit inline CRX tags anymore
     * they will be printed at the end of the BLIF as comments (#dual/#Triple)
     */
    if (pNode->vCoRoots.nSize > 1)
    {
        if (pNode->vCoRoots.nSize == 2 || pNode->vCoRoots.nSize == 3)
        {
            Io_CoRootAdd(pNode);
        }
    }
